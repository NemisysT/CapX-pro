...
export default function Sidebar() {
  ...
  const sidebarVariants = {
    expanded: { width: '16rem', transition: { duration: 0.5 } },
    collapsed: { width: '4rem', transition: { duration: 0.5 } }
  }
  ...
  return (
    <TooltipProvider>
      <motion.div 
        className="flex flex-col h-screen bg-gradient-to-br from-black via-gray-800 to-gray-900 border-r border-gray-700 shadow-lg"
        initial="expanded"
        animate={isCollapsed ? "collapsed" : "expanded"}
        variants={sidebarVariants}
      >
        <div className="flex items-center justify-between p-4">
          {!isCollapsed && <h1 className="text-2xl font-bold text-gray-300">CapX</h1>}
          <Button
            variant="ghost"
            size="icon"
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Link href={item.link}>
                      <Button
                        variant="ghost"
                        className={`w-full justify-start text-gray-300 ${isCollapsed ? 'px-2' : 'px-4'}`}
                      >
